...
void GetMappings (ifstream &Dnamemapin, HashTable &NameMapHT)
{
char Line[1000];
char *Token;
char s[2] = ";";
...
string Long;
   ...
   while (!Dnamemapin.eof())
   {
      ...
      Token = strtok(NULL, s);
      Short = Process(Token);
      NameMapHT.Insert(Long, Short); 
      Dnamemapin.getline (Line, 1000);
   }
}

//Apply the mappings in the HT to the Publishers
void ApplyMappings (str_vec_t Publishers[NUM_PUBS], HashTable &NameMapHT, str_vec_t ShortPubs[NUM_PUBS])
{
string Long;
string Short;
...
HashTable NameMapHT(1000);
   ...
   GetMappings (Dnamemapin, NameMapHT);
   ApplyMappings (Publishers, NameMapHT, ShortPubs);
